...
import { CustomTableBodyRow } from './components/custom-table-body-row';
import { CustomTableHeadRow } from './components/custom-table-head-row';
...
export const CustomTable = <T extends {}>(props: NCustomTable.IProps<T>) => {
  const {
    theadMainRowIndex = 0,
    tableTitle,
    tableName = '',
  ...
  const [tableHead, setTableHead] = React.useState<NCustomTable.ITableProps<T>['theadData']>([]);
          ...
          <Table className={joinClasses(classNameTable, ...classNames)}>
            <TableHead>
              {tableHead?.map((headRow, index) => {
                return <CustomTableHeadRow key={index} headRow={headRow} />;
              })}
            </TableHead>
...
// eslint-disable-next-line prettier/prettier
export const ContainerCustomTable = styled.div<NCustomTable.IStyle>`
