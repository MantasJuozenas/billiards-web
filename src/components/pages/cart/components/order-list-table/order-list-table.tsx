import {
  AddRemoveCartItem,
  ChangeCartItemQuantity
} from '@store/modules/cart/actions';
import { SVGIconEuro } from '@styles/global-icons/icons/svg-icon-euro';
import { SVGIconPlusIcon } from '@styles/global-icons/icons/svg-icon-plus-icon';
import { toArray } from '@utilsFn/dictionary';
import { useDispatch, useSelector } from '@utilsFn/hooks/use-selector';
import { useTranslation } from 'next-i18next';
import React from 'react';
import styled from 'styled-components';

export const OrderListTable = () => {
  const { t } = useTranslation('page-cart');

  const dispatch = useDispatch();
  const cartItems = useSelector((s) => s.cart.cartItems);
  const products = useSelector((s) => s.menu.menuList.products);
  const groupModifiers = useSelector((s) => s.menu.menuList.groupModifiers);

  const handlerOnClickChangeQuantity = (
    productId: string,
    index: number,
    dir: '-' | '+'
  ) => {
    dispatch(ChangeCartItemQuantity({ productId, index, action: dir }));
  };

  const handlerRemoveItem = (productId: string, index: number) => {
    dispatch(
      AddRemoveCartItem({
        productId,
        index,
        action: 'remove'
      })
    );
  };

  return (
    <ContainerOrderListTable>
      <thead>
        <tr>
          <th>{t('OrderListTable::text1')}</th>
          <th>{t('OrderListTable::text2')}</th>
          <th>{t('OrderListTable::text3')}</th>
          <th>{t('OrderListTable::text4')}</th>
        </tr>
      </thead>

      <tbody>
        {cartItems?.map?.((item, i) => {
          const productData = item?.product;
          const productName =
            productData?.ikos_product_json?.product?.name || '';
          const productPrice = (item?.price || 0)
            ?.toFixed?.(2)
            ?.replace?.('.', ',');

          const itemModifiers = toArray(item?.modifiers || {});

          return (
            <tr key={productData?.ikko_id}>
              <td>
                <div className="OrderListTable_item_title">
                  <div className="OrderListTable_item_name">{productName}</div>

                  {itemModifiers?.length ? (
                    <div className="OrderListTable_item_modifiers">
                      {itemModifiers?.map?.((modifier, ii) => {
                        const modifierData = products?.[modifier?.productId];
                        const modifierGroup =
                          groupModifiers?.[
                            modifierData?.ikos_product_json?.product?.groupId ||
                              ''
                          ];

                        return (
                          <div
                            key={`${modifier?.productId}_${ii}`}
                            className="OrderListTable_item_modifiers_item"
                          >
                            <p className="modifierGroupName">
                              {modifierGroup?.name}:
                            </p>

                            <p className="modifierName">
                              {modifierData?.ikos_product_json?.product?.name}
                            </p>
                          </div>
                        );
                      })}
                    </div>
                  ) : null}
                </div>
              </td>

              <td>
                <div className="OrderListTable_item_quantity">
                  <div
                    className="OrderListTable_item_minus"
                    onClick={() => {
                      handlerOnClickChangeQuantity(
                        productData?.ikko_id || '',
                        i,
                        '-'
                      );
                    }}
                  >
                    <div />
                  </div>

                  <div className="OrderListTable_item_count">
                    {item?.quantity}
                  </div>

                  <div
                    className="OrderListTable_item_plus"
                    onClick={() => {
                      handlerOnClickChangeQuantity(
                        productData?.ikko_id || '',
                        i,
                        '+'
                      );
                    }}
                  >
                    <SVGIconPlusIcon />
                  </div>
                </div>
              </td>

              <td>
                <div className="OrderListTable_item_price">
                  <SVGIconEuro />
                  {productPrice}
                </div>
              </td>

              <td>
                <div
                  className="OrderListTable_item_delete"
                  onClick={() => {
                    handlerRemoveItem(productData?.ikko_id || '', i);
                  }}
                >
                  {t('OrderListTable::text5')}
                </div>
              </td>
            </tr>
          );
        })}
      </tbody>
    </ContainerOrderListTable>
  );
};

const ContainerOrderListTable = styled.table`
  height: 1px;
  width: 100%;
  min-width: 770px;
  overflow: auto;
  border-collapse: separate; /* Don't collapse */
  border-spacing: 0;

  thead {
    tr {
      th {
        padding: 0 30px;
        padding-bottom: 34px;

        text-align: center;
        font: normal normal 400 16px/19px Roboto;
        letter-spacing: 0.4px;
        color: #a9a9a9;

        :first-child {
          width: 100%;

          padding-left: 0;

          text-align: left;
        }

        :last-child {
          padding-right: 0;

          text-align: right;
        }
      }
    }
  }

  tbody {
    tr {
      td {
        padding: 0 30px;
        padding-top: 20px;
        padding-bottom: 20px;

        border-bottom: 1px solid #383838;

        :first-child {
          width: 100%;

          padding-left: 0;

          text-align: left;
        }

        :last-child {
          padding-right: 0;

          text-align: right;
        }
      }
    }
  }

  .OrderListTable_item_title {
    //
  }

  .OrderListTable_item_name {
    font: normal normal 400 16px/19px Roboto;
    letter-spacing: 0.4px;
    color: #ffffff;
  }

  .OrderListTable_item_modifiers {
    margin-top: 15px;
  }

  .OrderListTable_item_modifiers_item {
    display: flex;
    align-items: center;

    :not(:last-child) {
      margin-bottom: 5px;
    }
  }

  .modifierGroupName,
  .modifierName {
    margin: 0;

    font: normal normal normal 14px/17px Roboto;
    letter-spacing: 0px;
    color: #ffffff;
  }

  .modifierName {
    margin-left: 5px;
  }

  .OrderListTable_item_quantity {
    display: flex;
    align-items: center;
  }

  .OrderListTable_item_minus,
  .OrderListTable_item_plus {
    cursor: pointer;

    min-width: 14px;
    width: 14px;
    max-width: 14px;
    min-height: 14px;
    height: 14px;
    max-height: 14px;

    display: flex;
    align-items: center;
    justify-content: center;
  }

  .OrderListTable_item_minus {
    div {
      width: 100%;
      height: 2px;

      border-radius: 20px;
      background-color: #ff0055;
    }
  }

  .OrderListTable_item_count {
    margin: 0 25px;

    font: normal normal 400 15px/14px Roboto;
    letter-spacing: 0.47px;
    color: #ffffff;
  }

  .OrderListTable_item_plus {
    //
  }

  .OrderListTable_item_price {
    display: flex;
    align-items: center;

    font: normal normal 400 16px/19px Roboto;
    letter-spacing: 0.4px;
    line-height: 1px;
    color: #ffffff;

    svg {
      width: 13px;
      height: 15px;

      margin-right: 5px;
    }
  }

  .OrderListTable_item_delete {
    cursor: pointer;

    font: normal normal 400 15px/18px Roboto;
    letter-spacing: 0.38px;
    color: #ffffff;
  }
`;
